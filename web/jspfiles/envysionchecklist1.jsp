...
<title>Envysion Checklist</title>
...
<%
  ...
  <body>
  <strong> Envysion Performance Report</strong><br><br>
  <% 
  ...
  %>
  <a href="envyperformanceupdate.jsp"><button>Update Performance</button></a>
  <%}%>
        <div class="mainbod">
          <form action="EnvysionResults" method="POST">
            <%
             if(envybean==null){
             %>
           <table id="envychecklist"><tr><th>Date Select</th><th>Start Date</th><th>End Date</th><th>Data Date</th><th>Auditor Name</th><th>Cafe Name</th><th>Task Name</th><td><input type="button" id="modify" value="MODIFY"></td></tr>
                <tr><td>
                        <ul id="tabrad" style="list-style-type:none"><li id="fli"> <input type="radio" id="serad" name="datesel" onclick="checkdate(this.value)" class="radiobut" value="sedate">Start & End</li>
                            <li id="sli"><input type="radio" id="dtrad" name="datesel" onclick="checkdate(this.value)" class="radiobut" value="dtdate">Data Date</li></ul>
                    </td><td><input type="text" class="sedate" id="stdate" autocomplete="off" name="performdate" disabled required></td>
                    <td><input type="text" class="sedate" id="endate" autocomplete="off" name="enddate" required disabled></td>
  ...
  %>
  <table id="envychecklist"><tr><th>Date Select</th><th>Start Date</th><th>End Date</th><th>Data Date</th><th>Auditor Name</th><th>Cafe Name</th><th>Task Name</th><td><input type="button" id="modify" onclick="modifysearch()" value="MODIFY"></td></tr>
                <tr><td>
                    <%if(datadate==null){%>
                    <input type="radio" id="serad" name="datesel" onclick="checkdate(this.value)" disabled checked class="radiobut" value="sedate">Start & End
                    <input type="radio" id="dtrad" name="datesel" onclick="checkdate(this.value)" disabled class="radiobut" value="dtdate">Data Date<%}
                    else{%>
                       <input type="radio" id="serad" name="datesel" onclick="checkdate(this.value)" disabled class="radiobut" value="sedate">Start & End
                   ...
                   <% }
                    ...
                    <td>
                      <%if(datadate==null){%>
                      <input type="text" class="sedate" id="endate" autocomplete="off" disabled name="enddate" value="<%=enDate%>" required><%}
                     else{%><input type="text" class="sedate" id="endate" autocomplete="off" name="enddate" disabled required><%}%>
                    </td>
            <td><%if(datadate==null){%>
...
<%}}}%>
 ...
 if(lauditorName.size()>0){
   ...
   <table id="keywords">
     <thead>
       <tr><th><span>S.No</span></th><%if(datadate!=null){%><th><span>Performed Date</span></th><th><span>Data Date</span></th><th><span>Task Count</span></th><%}else{}%><th><span>Auditor Name</span></th><th><span>Cafe Name</span></th><th><span>Task Name</span></th><th><span>Performance Time in Hours</span></th></tr></thead> 
     <tbody>     
...
%>
   ...
   <% }
if(datadate==null){%></tbody>
      <tr><td colspan="4" style="text-align: right"><strong>Total</strong></td><td><strong><%=dc.format(total)%></strong></td></tr>
   <%}
   else{}
 %></table></div>
 <%} 
 ...
 <%
              ...
              session.setAttribute("envyresults", defres);
             }
